...
export function getLoteamentoPublico(token) {
  return fetch(`/api-proxy/public-links/${encodeURIComponent(token)}`, {
    cache: 'no-store',
  }).then(async (res) => {
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      throw new Error(Array.isArray(body.message) ? body.message.join(', ') : body.message || 'Loteamento não encontrado');
    }
    return res.json();
  });
}
...
export function setLoteImagemCapa(dbId, imageUrl) {
  const filename = decodeURIComponent(String(imageUrl).split('/').pop());
  return request(`/lotes/${dbId}/imagens/${encodeURIComponent(filename)}/capa`, { method: 'PATCH' });
}
...
export function setApartamentoImagemCapa(apId, imageUrl) {
  const filename = decodeURIComponent(String(imageUrl).split('/').pop());
  return request(`/predios/apartamentos/${apId}/imagens/${encodeURIComponent(filename)}/capa`, { method: 'PATCH' });
}

export function createPublicPropertyLink(type, propertyId) {
  return request(`/public-links/${type}/${propertyId}`, { method: 'POST' });
}

export async function getPublicProperty(token) {
  const res = await fetch(`/api-proxy/public-links/${encodeURIComponent(token)}`, {
    cache: 'no-store',
  });
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    throw new Error(Array.isArray(body.message) ? body.message.join(', ') : body.message || 'Link invalido ou expirado');
  }
  return res.json();
}
